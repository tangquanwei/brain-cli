...
## 2026-08-24 13:40 CST

### 做了什么

- 新增全局 `--vault <path>` 和始终只读的 `brain doctor [path]`，已有 Markdown/Obsidian 知识库无需初始化或迁移即可体检。
- CLI 帮助和文档明确标记所有命令的读写边界；`links` 默认只读，只有 `--write` 写缓存，重命名和移动通过 `--dry-run` 只读预览。
- 链接图支持 Obsidian WikiLink 别名、笔记嵌入、图片/附件嵌入、标题及块引用；重命名或移动会同步重写已解析 WikiLink。
- 增加 1K/10K 合成知识库基准、完整测试和更新后的 WebUI 演示素材。

### 验证

- `npm test`：12 个测试文件、41 项测试通过。
- `npm run typecheck`、`npm run build` 通过。
- 公开 Demo Vault：10 篇笔记、16 条内部链接、0 断链、0 缺失引用、0 缺失附件、0 歧义 WikiLink、0 孤岛。
- 1K/10K 扫描分别为 58.0 ms / 1,377.6 ms，峰值 RSS 为 103.7 MB / 163.5 MB；测试库均位于系统临时目录并已删除。
